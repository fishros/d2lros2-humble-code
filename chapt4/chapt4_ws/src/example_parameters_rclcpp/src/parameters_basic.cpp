...
public:
  ...
  ParametersBasicNode(std::string name) : Node(name) {
    ...
    RCLCPP_INFO(this->get_logger(), "设置%s节点日志级别 %d .", name.c_str(), log_level);
    this->get_logger().set_level((rclcpp::Logger::Level)log_level);
    RCLCPP_INFO(this->get_logger(), "设置%s节点日志级别完成 %d .", name.c_str(), log_level);
...
private:
  ...
  void timer_callback()
  {
    RCLCPP_DEBUG(this->get_logger(), "设置%s节点日志级别 %d .", this->get_name(), log_level);
    RCLCPP_INFO(this->get_logger(), "设置%s节点日志级别 %d .", this->get_name(), log_level);
    
    this->get_logger().set_level((rclcpp::Logger::Level)log_level);

    RCLCPP_INFO(this->get_logger(), "设置%s节点日志级别完成 %d .", this->get_name(), log_level);
  }
